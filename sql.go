...
func (s *Stmt) QuerySlice(args []string) (*Rows, error) {
	s.closemu.RLock()
	defer s.closemu.RUnlock()
	...
	for i := 0; i < maxBadConnRetries; i++ {
		...
		rowsi, err = rowsiFromSlice(driverStmt{dc, si}, args)
		if err == nil {
			// Note: ownership of ci passes to the *Rows, to be freed
			// with releaseConn.
			rows := &Rows{
...
func rowsiFromSlice(ds driverStmt, dargs []string) (driver.Rows, error) {
	ds.Lock()
	want := ds.si.NumInput()
	ds.Unlock()
	if want != -1 && len(dargs) != want {
		return nil, fmt.Errorf("sql: statement expects %d inputs; got %d", want, len(dargs))
	}
	args, err := sliceargs(&ds, dargs)
	if err != nil {
		return nil, err
	}
	ds.Lock()
	rowsi, err := ds.si.Query(args)
	ds.Unlock()
	if err != nil {
		return nil, err
	}
...
func sliceargs(ds *driverStmt, args []string) ([]driver.Value, error) {
	dargs := make([]driver.Value, len(args))
	var si driver.Stmt
	if ds != nil {
		si = ds.si
	}
	cc, _ := si.(driver.ColumnConverter)
